...
func session(conn net.Conn) (err error) {
	...
	if !authorized {
		...
		welcome := "/asterisk_welcome"

		if agi.extension == "h" {
			if _, err = core.Chat(phone, "/hangup "+monitorFile, language); err != nil {
				return
			}
			return nil
		}
		...
		if message, err := core.Chat(phone, welcome, language); err != nil {
			return err
		...
		for {
			...
			if question != "" {
				...
				if answer, err = core.Chat(phone, question, language); err != nil {
					return
				}
